...
然后在 ABox 安装本插件。首次打开时，同意 **设备**、**Interconnect** 与 **接收 Interconnect 消息**权限。确认 ABox 已连接目标手表后，如插件先显示“未发现设备”，点击一次 **刷新设备**；插件会优先读取实时连接，再回退读取 ABox 已记录设备。状态显示已发现或已连接后，点击“选择图片并传输”，选取一张 JPG、JPEG、PNG、WEBP 或 GIF 图片。插件会在每个分片获得手表确认后继续发送，直至显示“图片已传入手表图库”。
