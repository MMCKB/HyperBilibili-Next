...
function updateManifestForLite(workspace) {
  ...
  Object.keys(pages).forEach((pageName) => {
    if (pageName === "pages/app/features/toolbox" || pageName.indexOf("pages/app/features/toolbox/") === 0) {
...
`;
  ...
  page = page.replace(/\n    OpenToolbox\(\)\{[\s\S]*?\n    \},\n    GoPage\(/, "\n    GoPage(");
  if (page.includes("OpenToolbox") || page.includes("colors.toolbox")) throw new Error("工具箱入口清理不完整，已停止生成精简包。");
...
function findRpk(directory) {
  ...
  visit(directory);
  if (matches.length !== 1) throw new Error("预期在 " + directory + " 找到一个 RPK，实际找到 " + matches.length + " 个。");
  return matches[0];
}
